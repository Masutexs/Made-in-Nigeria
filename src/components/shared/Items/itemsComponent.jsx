...
export default function ItemsComponent() {
  return (
    <>
      {items.map(item =>
        <div key={item.id} className='w-[200px] h-[339px] bg-white rounded-[5px] shadow  border-black border-opacity-10' >
          ...
          <button className="mt-3 w-[177px] h-8 px-2 py-1.5 bg-green-600 rounded-[5px] border border-green-700 border-opacity-80 flex-col justify-center items-center gap-2.5 inline-flex text-black text-[13px] font-normal font-['Poppins']">
            Add Cart
          </button>
          <div className="pl-3 text-left text-black text-[11px] font-normal font-['Poppins']">Ratings:</div>
        </div>
      )}
    </>
  )
